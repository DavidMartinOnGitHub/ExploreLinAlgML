...
def SGConvolution(x, filter):
    
    if isinstance(x, pd.core.frame.DataFrame):
        return SGConvolution(np.array(x), filter);
    
    
    if len(x.shape) == 1:
        # np.flip reverses the filter definition, allowing filter to
        # be specified in standard commonly accepted form.
        Xsg = np.convolve(x, np.flip(filter), mode='valid');
    elif len(x.shape) == 2:
        Xsg = [];
        rows = x.shape[0];
        for row in range(0, (rows)):
            data = x[row];
            Xsg.append(SGConvolution(data, filter));
        Xsg = np.array(Xsg);
    else:
        # print(x.shape)
        # print(len(x.shape))
...
# standard format of an SG filter
filter = np.array([-2.0, -1.0, 0.0, +1.0, +2.0])/10.0;


#%%
X0 = np.array(X);
X1 = SGConvolution(X0, filter);
Xpta = SNV(X1);

assert np.allclose(Xpte, Xpta, rtol=0.0, atol=1e-6)

#%%
Xpta = SNV(SGConvolution(X, filter));
...
assert np.allclose(SGConvolution(X0, filter), SGConvolution(X, filter), rtol=0.0, atol=1e-6)

assert np.allclose(SNV(X0[0]), SNV(X.iloc[0,:]), rtol=0.0, atol=1e-6)
assert np.allclose(SGConvolution(X0[0], filter), SGConvolution(X.iloc[0,:], filter), rtol=0.0, atol=1e-6)
